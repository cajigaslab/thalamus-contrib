...
impl<'a> AnalogData for SleeveData<'a> {
  fn short_data(&self, channel: i32) -> &[i16] {
    match self.to_type_index(channel) {
      (ChannelType::INTAN, i) => self.intan_channels[i].as_slice(),
      (ChannelType::ICM, i) => self.icm_channels[i].as_slice(),
      (ChannelType::ADC, i) => self.adc_channels[i].as_slice(),
    }
  }

  fn num_channels(&self) -> i32 {
    (self.intan_channels.len() + self.icm_channels.len() + self.adc_channels.len()) as i32
  }

  fn sample_interval(&self, channel: i32) -> std::time::Duration {
    match self.to_type_index(channel) {
      (ChannelType::INTAN, _) => Duration::from_millis(1),
      (ChannelType::ICM, _) => Duration::from_millis(10),
      (ChannelType::ADC, _) => Duration::from_millis(10),
    }
  }

  fn name(&self, channel: i32) -> &str {
    match self.to_type_index(channel) {
      (ChannelType::INTAN, i) => match i {
        0 => "INTAN:0",
        1 => "INTAN:1",
        2 => "INTAN:2",
        3 => "INTAN:3",
  ...
  fn scale(&self, channel: i32) -> f64 {
    match self.to_type_index(channel) {
      (ChannelType::INTAN, _) => 0.000195,
      (ChannelType::ICM, _) => 1.0,
      (ChannelType::ADC, _) => {
...
fn stop_bluetooth<F: FnOnce() + 'static>(inner: Arc<Mutex<SleeveNodeInner>>, on_stopped: F) {
  let (handle, api, main_thread_token) = {
    ...
    (
      lock.bluetooth_handle.take(),
      lock.api,
      lock.main_thread_token,
    )
  };
...
impl SleeveNodeInner {
  async fn bluetooth(
    ...
    let manager = get_result!(
      wait(bt::get_manager(), &cancel_token).await,
      "Manager init failed"
    );
    let adapters = get_result!(
      wait(manager.adapters(), &cancel_token).await,
      "Failed to get adapters"
    );
    let central = get_option!(adapters.into_iter().nth(0), "No adapters found");
    get_result!(
      wait(central.start_scan(ScanFilter::default()), &cancel_token).await,
      "start_scan failed"
    );
    ...
    let scan_handle = api.tokio().as_ref().unwrap().spawn(async move {
      ...
      let peripherals = get_result!(
        wait(events_central.peripherals(), &events_cancel_token).await,
        "Failed to query existing peripherals"
      );
      println!("Previous peripherals");
      for peripheral in peripherals {
        let pid = peripheral.id();
        let found = get_result!(
          wait(handle_peripheral(peripheral), &events_cancel_token).await,
          "Failed to handle peripheral"
        );
        if found {
      ...
      let mut events = get_result!(
        wait(events_central.events(), &events_cancel_token).await,
        "Failed to get events"
      );
      while let Some(e) = wait_opt(events.next(), &events_cancel_token).await {
        match e {
          CentralEvent::DeviceDiscovered(id) => {
            let peripheral = get_result!(
              wait(events_central.peripheral(&id), &events_cancel_token).await,
              "Failed to get peripheral"
            );
            let found = get_result!(
              wait(handle_peripheral(peripheral), &events_cancel_token).await,
              "Failed to handle peripheral"
    ...
    get_result!(
      wait(peripheral.connect(), &cancel_token).await,
      "Connect failed"
    );
    ...
    get_result!(
      wait(peripheral.discover_services(), &cancel_token).await,
    ...
    get_result!(
      wait(peripheral.subscribe(&tx_char), &cancel_token).await,
      "Failed to subscript to TX"
    );
...

    {
      ...
      get_result!(
        wait(
          peripheral.write(&rx_char, &block.encode(), WriteType::WithoutResponse),
          &cancel_token
        )
        .await,
        "Write failed"
    ...
    let mut notifications = get_result!(
      wait(peripheral.notifications(), &cancel_token).await,
...
impl Node for SleeveNode {
  fn new(
    ...
    let inner = Arc::new_cyclic(|weak: &std::sync::Weak<Mutex<SleeveNodeInner>>| {
      ...
      let state_callback =
        move |source: State, action: StateAction, key: StateValue, value: StateValue| {
    ...
    state.recap_with(
      |source: State, action: StateAction, key: StateValue, value: StateValue| {
